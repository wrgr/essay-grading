...
class ScenarioRunner:
    ...
    # ─── Serialisation (platform addition) ──────────────────────────────────
    # The V5 app kept runners in an in-memory dict, so a restart lost every
    # in-flight assessment. All phase state is plain lists/dicts/strings, so
    # runners round-trip through assessment_runs.state_json: load -> from_dict
    # -> respond() -> to_dict -> save, one cycle per turn.

    _STATE_FIELDS = ("history", "user_inputs", "writing_metrics", "phase",
                     "recall_history", "probe_history", "probe_queue",
                     "probe_index", "_probe_followup_pending")

    def to_dict(self):
        state = {f: getattr(self, f) for f in self._STATE_FIELDS}
        # probe_results is keyed by int; JSON round-trips keys as strings
        state["probe_results"] = {str(k): v for k, v in self.probe_results.items()}
        return state

    @classmethod
    def from_dict(cls, state, scenario, model, api_key, base_url):
        runner = cls(scenario, model, api_key, base_url)
        for f in cls._STATE_FIELDS:
            if f in state:
                setattr(runner, f, state[f])
        runner.probe_results = {int(k): v for k, v in (state.get("probe_results") or {}).items()}
        return runner
